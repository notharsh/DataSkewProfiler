...
link_rec show_actor(test.KeysKevinBacon.ActorLinks L) := TRANSFORM
  SELF.workedInMovie := L.movie; // the movie the actor worked in
  SELF := L;
END;

link_rec show_movie(test.KeysKevinBacon.MovieLinks L) := TRANSFORM
  SELF.workedInMovie := L.movie;  // the movie the actor worked in
  SELF := L;
END;
...
output_links (STRING find_actor) := FUNCTION
  ...
  ds := PROJECT(test.KeysKevinBacon.ActorLinks(actor=find_actor), show_actor(LEFT)) ;
  // return the next level actor using MovieLinks index passing it the
  // movie retrieved via ActorLink index
  RETURN SORT(ds + PROJECT(test.KeysKevinBacon.MovieLinks(movie = ds[1].workedInMovie), show_movie(LEFT)),-level) ;
END;

search_KB_links(STRING find_actor) := FUNCTION
  // what level is the actor in
  level := test.KeysKevinBacon.ActorLinks(actor = find_actor)[1].level;
